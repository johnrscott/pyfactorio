...
class Recipe:
    ...
    def __init__(self, item_dataframe):
        ...
        self.ingredients = dict(zip(item_dataframe["resource"], item_dataframe["quantity"]))
        
    def __repr__(self):
        """
        Get string representation
        """
        return (f"Recipe for {self.num_produced} {self.item} in {self.handcraft_time}s, " +
    ...
    def assemblers_required(self, throughput, assembler_crafting_speed = 1):
        ...
        """
        return throughput * self.recipe_time(assembler_crafting_speed) / self.num_produced
...
class RecipeList:
    ...
    def __init__(self, factorio_resources_ods):
        factorio_resources = pd.read_excel(factorio_resources_ods, engine="odf")
        self.recipes = {}
    ...
    def get_raw_material_counts(self, item, raw_materials):
        ...
        for ingredient, num_required in item_recipe.ingredients.items():
            ...
            if ingredient in raw_materials:
                ...
                ingredient_raw_materials = self.get_raw_material_counts(ingredient, raw_materials)
                scaled_ingredient_raw_materials = scale_dictionary(ingredient_raw_materials, num_required)

            all_raw_materials = add_dictionaries(all_raw_materials, scaled_ingredient_raw_materials)
        
        return scale_dictionary(all_raw_materials, 1.0/item_recipe.num_produced)
    ...
    def get_item_dependencies(self, item, raw_materials):
        ...
        for ingredient in item_recipe.ingredients:
            ...
            if ingredient in raw_materials:
                ...
                item_dependencies[ingredient] = self.get_item_dependencies(ingredient, raw_materials)
    ...
    def get_recipe(self, item):
        ...
        try:
            ...
            raise ValueError(f"Item {item} does not exist in the recipes list. Check the resources .ods file.")

    def ingredient_assemblers_per_recipe(self, item, assembler_crafting_speed, raw_materials):
        """
        ...
        for ingredient, num_required in item_recipe.ingredients.items():
            ...
            assembler_counts[ingredient] = ingredient_recipe.assemblers_required(throughput,
...
class AssemblerTree:
    ...
    def __init__(self, item, throughput, assembler_crafting_speed, recipes, raw_materials):
        ...
        if item in raw_materials:
            ...
            item_recipe_time = item_recipe.recipe_time(assembler_crafting_speed)
            self.num_assemblers = item_recipe.assemblers_required(throughput, assembler_crafting_speed)
            ...
            for ingredient, num_required in item_recipe.ingredients.items():
                ...
                ingredient_assembler_tree = AssemblerTree(ingredient, ingredient_output_throughput,
    ...
    def total_raw_input_throughput(self):
        ...
        if self.is_raw_material():
            ...
            for ingredient_assembler_tree in self.ingredients:
                total_throughput = add_dictionaries(total_throughput,
